    ...
    try {
        ...
        for (let i = 0; i < restData.length; i++) {
            try {
                ...
                setAttr(['type', 'class', 'data-toggle', 'data-placement'], ['button', 'btn btn-primary', 'tooltip', 'top'], butt);
                butt.innerText = 'Click for Weather';
                cardbody.append(butt);
                if (restData[i].capital == '') {
                    butt.title = `Weather not available for this city`
                    continue;
                }
                butt.title = `Temperature: ${weatherData.main.temp}
